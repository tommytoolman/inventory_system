...
class ProductMatcher:
    ...
    async def find_potential_matches(self, min_confidence: int = 85, status: str = "ACTIVE",
        ...
        for product1 in platform1_products:
            ...
            # If no exact brand match, try similar brands (optional, increases computation)
            # Consider making this threshold configurable or optional
            # if not matching_products:
            #     for brand, products in brand_groups.items():
            #         brand_similarity = fuzz.ratio(brand1, brand)
            #         if brand_similarity >= 85:
...
# Example usage (typically called from a CLI script):
# async def main():
...
#     async with async_session_factory() as session:
#         matcher = ProductMatcher(session, DATABASE_URL)

#         # 1. Find potential matches
#         potential_groups = await matcher.find_potential_match_groups(min_confidence=75, status="ACTIVE")
#         print(f"Found {len(potential_groups)} potential groups.")

#         # 2. (Manual Step) Review potential_groups, create confirmed_matches list
#         # confirmed_matches = [...] # Populate this based on review
#         # matcher.save_progress(confirmed_matches, processed_pairs) # Example save

#         # 3. Merge confirmed matches
#         # loaded_matches, _ = await matcher.load_progress() # Example load
#         # if loaded_matches:
#         #     await matcher.merge_products(loaded_matches)

#         # 4. Restore example
#         # restore_result = await matcher.restore_merged_product(merged_product_id=12345)
#         # if restore_result["success"] and restore_result["potential_platform_common"]:
#         #    ids_to_relink = [pc["id"] for pc in restore_result["potential_platform_common"] if pc["platform"] == 'reverb'] # Example selection
#         #    await matcher.restore_platform_common(restore_result["restored_product_id"], ids_to_relink)


# if __name__ == "__main__":
#     # Make sure environment variables are loaded if using .env file
#     # from dotenv import load_dotenv
#     # load_dotenv()
#     asyncio.run(main())