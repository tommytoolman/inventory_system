...
- [ ] **Stocked inventory quantity sync investigation** – _Progress 2026-01-26:_ Fixed multiple stocked item bugs: (1) Inventory reconciliation endpoint now correctly updates Shopify quantities (removed save/restore logic in Reverb block), (2) Added status filter to skip quantity_change sync events for ended/sold eBay listings, (3) Added Skip button for non-Reverb new_listing events to handle legitimate Shopify-only products, (4) Fixed duplicate sync event creation by checking for existing events regardless of status. **Remaining:** User reports there's still a bug with stocked inventory synchronization that needs investigation. Possible areas: (a) Quantity propagation timing/ordering across platforms, (b) Edge cases in OrderSaleProcessor for stocked items, (c) Race conditions in concurrent quantity updates.
