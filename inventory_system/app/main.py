...
from app.routes.platforms.woocommerce import router as woocommerce_router, webhook_router as wc_webhook_router, store_router as wc_store_router
...
async def lifespan(app: FastAPI):
    # ------------------------------------------------------------------ #
    # LOGGING SETUP — must happen first so every subsequent log record    #
    # (including startup messages) is captured by both handlers.          #
    # ------------------------------------------------------------------ #
    import logging as _logging
    from app.services.log_review_service import LogAggregatorHandler, DailyLogReviewScheduler
    from app.core.discord_logger import DiscordLogHandler

    # 1. Daily-email aggregator (existing handler — unchanged)
    ...
    # 2. Discord real-time alert handler
    #    Reads DISCORD_WEBHOOK_URL / DISCORD_SERVICE_NAME / DISCORD_LOGGING_ENABLED
    #    from environment variables (or Railway dashboard variables).
    #    Only WARNING and above are sent to Discord — INFO/DEBUG stay console-only.
    _settings = get_settings()
    discord_handler = DiscordLogHandler(
        webhook_url=_settings.DISCORD_WEBHOOK_URL or None,
        service_name=_settings.DISCORD_SERVICE_NAME,
        environment=_settings.ENVIRONMENT,
        batch_interval=_settings.DISCORD_BATCH_INTERVAL,
        queue_limit=_settings.DISCORD_QUEUE_LIMIT,
        # WARNING+ goes to Discord; DEBUG/INFO stay console-only to reduce noise
        min_level=_logging.WARNING,
    )
    _logging.getLogger().addHandler(discord_handler)
    app.state.discord_handler = discord_handler  # expose for manual flush if needed

    # Start the background batch sender (runs every DISCORD_BATCH_INTERVAL seconds).
    # Non-blocking — if Discord is unreachable the app continues unaffected.
    discord_handler.start_batch_processor()
    ...
    try:
        ...
        # Stop Discord batch sender and flush any remaining queued records
        # before the process exits.  This is best-effort — if the webhook
        # is unreachable the shutdown still completes normally.
        discord_handler.stop_batch_processor()
        await discord_handler.flush_now()
...
app.include_router(wc_store_router, dependencies=[require_auth()])  # WooCommerce store management
