...
async def dashboard(request: Request, username: str = Depends(get_current_username)):
    ...
    async with async_session() as db:
        try:
            ...
            # Recent sync errors for dashboard widget
            recent_sync_errors = (
                await db.execute(
                    select(SyncErrorRecord)
                    .where(SyncErrorRecord.resolved == False)
                    .order_by(SyncErrorRecord.created_at.desc())
                    .limit(5)
                )
            ).scalars().all()
            unresolved_error_count = len(recent_sync_errors)
