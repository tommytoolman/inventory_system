...
from app.services.error_logger import SyncErrorLogger
    ...
    async def propagate_product_edit(
        ...
        *,
        ...
        if tasks:
            ...
            error_logger = SyncErrorLogger(self.db)
            for platform_name, response in zip(task_platforms, responses):
                if isinstance(response, Exception):
                    ...
                    try:
                        await error_logger.log_upload_error(
                            product_id=product_id,
    ...
    async def _run_vr_update_background(
        ...
        async with semaphore:
            ...
            try:
                ...
                try:
                    await SyncErrorLogger(self.db).log_upload_error(
                        product_id=None,
                        platform="vr",
                        operation="update",
                        error=exc,
                        extra_context={"external_id": external_id},
                    )
                except Exception:
                    pass
