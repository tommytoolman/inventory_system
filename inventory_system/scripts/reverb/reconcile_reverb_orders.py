#!/usr/bin/env python3
"""
Reconcile Reverb orders against product status.

This script compares Reverb orders in the database against product status to identify:
1. Shipped orders for ACTIVE single-item products (missed sales)
2. Shipped orders for inventorised products (qty adjustments needed)
3. Cancelled/refunded orders for SOLD products (potential relists)

Usage:
    python scripts/reverb/reconcile_reverb_orders.py [--fix] [--verbose]

Examples:
    # Dry run - just show discrepancies
    python scripts/reverb/reconcile_reverb_orders.py

    # Verbose output with details
    python scripts/reverb/reconcile_reverb_orders.py --verbose

    # Apply fixes for inventorised items (decrements qty)
    python scripts/reverb/reconcile_reverb_orders.py --fix
"""
...
async def get_discrepancies(verbose: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    """
    Identify discrepancies between Reverb orders and product status.

    Returns dict with:
        - missed_sales: Shipped orders where single-item product is still ACTIVE
        - inventorised_adjustments: Shipped orders for inventorised products needing qty sync
        - potential_relists: Cancelled/refunded orders where product is SOLD
    ...
    async with async_session() as db:
        ...
        for order in orders:
            order_dict = {
                ...
                "sku": order.sku,
                "title": order.title[:60] + "..." if order.title and len(order.title) > 60 else order.title,
            ...
            # SHIPPED/RECEIVED orders
            if order.order_status in ("shipped", "received"):
            ...
            elif order.order_status in ("cancelled", "refunded"):
                if order.product_status == "SOLD":
                    # Product marked sold but order was cancelled/refunded
...
def print_report(results: Dict[str, List], verbose: bool = False):
    ...
    print("\n" + "=" * 80)
    print("REVERB ORDERS RECONCILIATION REPORT")
    print(f"Generated: {datetime.now().isoformat()}")
    print("=" * 80)

    # Summary
    print("\n## SUMMARY")
    print(f"  Missed sales (single-item ACTIVE after ship):  {len(results['missed_sales'])}")
    print(f"  Inventorised qty adjustments needed:           {len(results['inventorised_adjustments'])}")
    print(f"  Potential relists (cancelled/refunded SOLD):   {len(results['potential_relists'])}")
    ...
    if results["missed_sales"]:
        ...
        print("   These orders shipped but product was never marked SOLD")
        print("-" * 80)
        for item in results["missed_sales"]:
            print(f"  Order #{item['order_number']} | {item['order_status']}")
    ...
    if results["inventorised_adjustments"]:
        ...
        for item in results["inventorised_adjustments"]:
            ...
            print(f"    Title: {item['title']}")
            print(f"    Action: Decrement by {item['decrement_by']} -> sync to eBay/Shopify")
            print()

    # Potential Relists - USER REVIEW
    if results["potential_relists"]:
        print("\n" + "-" * 80)
        print("## POTENTIAL RELISTS (Cancelled/Refunded but product is SOLD)")
    ...
    if verbose and results["ok_cancelled"]:
        ...
        for item in results["ok_cancelled"][:5]:  # Show first 5 only
            print(f"  Order #{item['order_number']} | {item['order_status']} | Product: {item['product_status']} | OK")
...
async def main():
    parser = argparse.ArgumentParser(description="Reconcile Reverb orders against product status")
