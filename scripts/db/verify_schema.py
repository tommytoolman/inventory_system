...
async def verify_schema():
    ...
    async with engine.connect() as conn:
        # Get database table info using run_sync
        def get_db_columns(sync_conn):
            inspector = inspect(sync_conn)
            return {col["name"]: col for col in inspector.get_columns("reverb_listings")}
        
        db_columns = await conn.run_sync(get_db_columns)
