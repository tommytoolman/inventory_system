...
async def find_and_review_matches(matcher, confirmed_matches, processed_pairs):
    ...
    try:
        ...
        # NEW 060625: Brand filter option
        brand_filter = input("Filter by brand (optional, press Enter for all brands): ").strip()
        if not brand_filter:
            brand_filter = None
        else:
        ...
        # Find potential matches WITH BRAND FILTER
...
async def export_matches_to_excel(matcher):
    ...
    with pd.ExcelWriter(f"price_analysis_{platform1}_vs_{platform2}_{timestamp}.xlsx", engine='openpyxl') as writer:
        ...
        df.sort_values(by=['confidence'], 
            ascending=False).to_excel(writer, sheet_name="All By Confidence", index=False)
