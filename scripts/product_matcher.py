...
class ProductMatcher:
    ...
    async def find_potential_matches(self, min_confidence: int = 85, status: str = "ACTIVE",
                                    platform1: str = "reverb", platform2: str = "ebay",
                                    brand_filter: str = None) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Finding potential matches between {platform1} and {platform2} "
                ...
                f"{f', brand={brand_filter}' if brand_filter else ''})")
        
        # Get excluded product IDs from merge history
        excluded_product_ids = await self._get_excluded_product_ids()
        
        # Get active products grouped by platform WITH BRAND FILTER
        products_by_platform = await self._get_products_by_platform(status, brand_filter)
        ...
        platform1_products = [p for p in platform1_products if p['id'] not in excluded_product_ids]
        platform2_products = [p for p in platform2_products if p['id'] not in excluded_product_ids]
        
        logger.info(f"After excluding merged products: {len(platform1_products)} products for {platform1}, "
            f"{len(platform2_products)} products for {platform2}")
    ...
    def _calculate_match_confidence_old(self, product1: ProductDict, product2: ProductDict) -> float:
        """
    ...
    async def _get_products_by_platform(self, status: str = "ACTIVE", brand_filter: str = None) -> Dict[str, List[ProductDict]]:
        """
        ...
        if brand_filter:
            where_conditions.append("LOWER(p.brand) LIKE LOWER(:brand_filter)")
        ...
        vintageandrare_prices AS (
            SELECT pc.id as platform_common_id, vl.price_notax as price
            FROM platform_common pc
            JOIN vr_listings vl ON pc.external_id = vl.vr_listing_id
            WHERE pc.platform_name = 'vintageandrare'
        ),
        shopify_prices AS (
            SELECT pc.id as platform_common_id, sl.price
            FROM platform_common pc
        ...
        WHERE {where_clause}
        ORDER BY p.id, pc.platform_name
        """)

        logger.debug(f"Executing _get_products_by_platform with status: {status}"
                    f"{f', brand_filter: {brand_filter}' if brand_filter else ''}")
        
        result = await self.db.execute(query, params)  # Use params instead of just status
        ...
        for row in rows:
            ...
            products_by_platform[platform].append({
                ...
                'brand': brand,
                'model': model,
                'title': row.get('title') or f"{brand} {model}".strip(),  # Use actual title or generate
                'year': row.get('year'),  # Add year field                'description': row['description'],
    ...
    async def _get_excluded_product_ids(self) -> Set[int]:
        """
        Get product IDs that should be excluded from matching.
        
        Returns:
            Set of product IDs to exclude:
            - Products that have been merged away (merged_product_id)
            - Products that have been kept in multiple merges (fully consolidated)
        """
        query = text("""
            SELECT DISTINCT product_id FROM (
                -- Products that were merged away
                SELECT merged_product_id as product_id FROM product_merges
                UNION
                -- Products that were kept in ANY merge (should be done matching)
                SELECT kept_product_id as product_id FROM product_merges
            ) excluded_products
        """)
        
        result = await self.db.execute(query)
        excluded_ids = {row[0] for row in result.fetchall()}
        logger.debug(f"Excluding {len(excluded_ids)} products from matching based on merge history")
        return excluded_ids
    ...
    def normalize_brand(self, brand: str) -> str:
        ...
        Args:
            brand: Raw brand name from product data
        ...
        # Common brand normalizations for your guitar/music equipment business
        brand_mappings = {
            # Fender variations
            'fender usa': 'fender',
            'fender american': 'fender',
            'fender custom shop': 'fender',
            
            # Gibson variations  
            'gibson usa': 'gibson',
            'gibson custom': 'gibson',
            ...
            # Add more mappings specific to your inventory
        }
        
        return brand_mappings.get(normalized, normalized)

    def _extract_year_from_title(self, title: str) -> Optional[int]:
        """
        Extract year from product title (first 4 characters if they form a valid year).
        ...
        try:
            potential_year = int(title[:4])
            # Reasonable year range for vintage instruments
    ...
    def _calculate_match_confidence(self, product1: ProductDict, product2: ProductDict) -> float:
        ...
        Prioritizes: Brand + Price + Year matching for 100% confidence.
        ...
        brand1 = self.normalize_brand(product1.get('brand', ''))
        brand2 = self.normalize_brand(product2.get('brand', ''))
        title1 = product1.get('title', '').lower()
        title2 = product2.get('title', '').lower()
        
        # Extract years - use database field first, then title extraction
        year1 = product1.get('year') or self._extract_year_from_title(product1.get('title', ''))
        year2 = product2.get('year') or self._extract_year_from_title(product2.get('title', ''))
        
        # --- Brand Matching (REQUIRED) ---
        if not brand1 or not brand2:
            logger.debug(f"Missing brand data: '{brand1}' vs '{brand2}'")
            return 0.0
        
        brand_match = brand1 == brand2
        brand_similarity = fuzz.ratio(brand1, brand2)
        
        # Require strong brand similarity
        if brand_similarity < 80:
            logger.debug(f"Low brand similarity ({brand_similarity}): '{brand1}' vs '{brand2}'")
            return max(0.0, brand_similarity * 0.3)  # Very low score for poor brand match
        ...
        price_match = False
        price_score = 50.0  # Default neutral score
        ...
        if price1 > 0 and price2 > 0:
            ...
            # Price matching thresholds
            if price_diff_pct <= 1.0:  # Within 1% = exact match
                price_match = True
                price_score = 100.0
            elif price_diff_pct <= 5.0:  # Within 5% = very close
                price_score = 95.0
            elif price_diff_pct <= 10.0:  # Within 10% = close
                price_score = 85.0
            elif price_diff_pct <= 20.0:  # Within 20% = acceptable (platform fees)
                price_score = 70.0
            else:
                price_score = max(0.0, 50.0 - price_diff_pct)  # Decreasing score
        
        # --- Year Matching ---
        year_match = False
        year_score = 50.0  # Default neutral score
        
        if year1 and year2:
            if year1 == year2:
                year_match = True
                year_score = 100.0
            elif abs(year1 - year2) == 1:  # Off by one year
                year_score = 90.0
            elif abs(year1 - year2) <= 2:  # Within 2 years
                year_score = 80.0
            else:
                year_score = max(0.0, 50.0 - abs(year1 - year2) * 5)  # Penalty for year difference
        
        # --- SKU Matching (if available) ---
        sku1 = str(product1.get('sku', '')).strip()
        sku2 = str(product2.get('sku', '')).strip()
        sku_match = False
        
        if sku1 and sku2 and sku1 == sku2:
            sku_match = True
            # Same SKU = definitely same product
            logger.debug(f"SKU match found: {sku1}")
            return 100.0
        
        # --- Perfect Match Conditions ---
        # Brand + Price + Year all match = 100% confidence
        if brand_match and price_match and year_match:
            logger.debug(f"Perfect match: brand={brand1}, price_diff={abs_diff:.2f}, year={year1}")
            return 100.0
        
        # --- Title Similarity (fallback) ---
        title_similarity = fuzz.token_sort_ratio(title1, title2) if title1 and title2 else 0
        
        # --- Model Matching (if available) ---
        model1 = product1.get('model', '').lower()
        model2 = product2.get('model', '').lower()
        model_similarity = fuzz.ratio(model1, model2) if model1 and model2 else 50
        
        # --- Weighted Final Score ---
        # If we don't have a perfect match, calculate weighted average
        scores = []
        weights = []
        
        # Brand is most important (already filtered above)
        scores.append(brand_similarity)
        weights.append(0.3)
        ...
        # Year is important for vintage items
        ...
        # Model similarity
        scores.append(model_similarity)
        weights.append(0.05)
        
        # Calculate weighted average
        weighted_score = sum(score * weight for score, weight in zip(scores, weights))
        
        logger.debug(f"Match scores - Brand: {brand_similarity:.1f}, Price: {price_score:.1f}, "
                    f"Year: {year_score:.1f}, Title: {title_similarity:.1f}, Final: {weighted_score:.1f}")
